...
doctor = Doctor(email='admin@admin.com',ssn=30112040088445, Fname='ahmed', Lname='mohamed', gender='Male', user_role='Admin', speciallity='Emergency',password=hashed, salary=1000000)

db.session.add(doctor) 
db.session.commit()

hashed = bcrypt.generate_password_hash('doctor').decode('utf-8')
doctor = Doctor(email='doctor@doctor.com',ssn=32112040088445, Fname='D.maahas', Lname='mohamed', gender='male', user_role='Doctor', password=hashed, speciallity='oncology' ,salary=1000000)
...
report1 = Report(patient_id= 3,doctor_id = 1,title= "kosomk" ,statement= "he will die soon")
report2 = Report(patient_id= 3,doctor_id = 2,title='kosomen omk' ,statement= "al23mar bed allah")
